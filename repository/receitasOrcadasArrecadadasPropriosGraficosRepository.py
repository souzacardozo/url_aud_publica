...
from model.receitasOrcadasArrecadadasPropriosGraficos import ReceitasOrcadasArrecadadasPropriosGraficos

class ReceitasOrcadasArrecadadasPropriosGraficosRepository:
    def __init__(self):
        self.conexao = ConexaoClickhouse()
        
    def obter_receitasOrcadasArrecadadasPropriosGraficos(self, entidades, idquadrimestres, ano): 
    ...
    # Join the values in each list separately
        ...
        query = """
                SELECT  dsclassificacaoreceita,
                    ...
                    SELECT
                                ...
                                0 AS valorTotal,
                                0 AS percentual
                            FROM {}.aud_receita_orcada_realizada_proprias)valorRealizadoss
                            WHERE cdentidade = 1
                                AND idquadrimestre IN ({}) 
                                AND nrano = {}
        ...
        # Process the results
        receitas = [ReceitasOrcadasArrecadadasPropriosGraficos(r[0], r[1], r[2]) for r in resultado]
